...
module VrpConfiguration
  ...
  params :vrp_request_partition do
    ...
    optional(:entity, type: Symbol, values: [:vehicle, :work_day], desc: 'Describes what the partition corresponds to. Only available if method in [balanced_kmeans hierarchical_tree].', coerce_with: ->(value) { value.to_sym })
    optional(:threshold, type: Integer, desc: 'Maximum size of partition. Only available if method in [iterative_kmean clique].')
  end
  ...
  params :vrp_request_preprocessing do
    ...
    optional(:kmeans_centroids, type: Array[Integer], desc: 'Forces centroid indices used to generate clusters with kmeans partition_method. Only available with deprecated partition_method.')
...
module VrpVehicles
  ...
  params :vehicle_model_related do
    ...
    optional(:sequence_timewindow_ids, type: String, documentation: { hidden: true }, desc: 'Sequence timewindows to consider, CSV front only')
  ...
  params :router_options do
    ...
    optional :speed_multiplier_area, type: Array[Float], coerce_with: ->(c) { c.is_a?(String) ? c.split(/;|\|/).collect{ |f| Float(f) } : c }, desc: 'Speed multiplier per area, 0 to avoid area. Areas separated with pipes (only available for truck mode at this time).'
