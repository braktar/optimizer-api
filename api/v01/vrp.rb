      ...
      namespace :vrp do # rubocop:disable Metrics/BlockLength
        ...
        resource :jobs do # rubocop:disable Metrics/BlockLength
          ...
          get ':id' do # rubocop:disable Metrics/BlockLength
            ...
            if job_object.nil? && (job.nil? || job.killed? || Resque::Plugins::Status::Hash.should_kill?(id) || job['options']['api_key'] != params[:api_key])
              status 404
              error!({ message: "Job with id='#{id}' not found" }, 404)
            end

            job_object ||= OptimizerWrapper::Result.get(id) || {}
            output_format = params[:format]&.to_sym ||
                            (job_object[:configuration] && job_object[:configuration][:csv] ? :csv : env['api.format'])
            ...
            if job&.completed? # job can still be nil if we have the solution from the dump
              APIBase.dump_vrp_dir.write([id, params[:api_key], 'solution'].join('_'), Oj.dump(job_object)) if stored_result.nil? && OptimizerWrapper.config[:dump][:solution]
            ...
            if output_format == :csv && (job.nil? || job.completed?) # At this step, if the job is nil then it has already been retrieved into the result store
              present(OutputHelper::Result.build_csv(job_object[:result].map(&:deep_symbolize_keys!)), type: CSV)
            else
              present({
                solutions: job_object[:result],
                job: {
                  id: id,
                  status: job&.status&.to_sym || :completed, # :queued, :working, :completed, :failed
                  avancement: job&.message,
                  graph: job_object[:graph]
                },
                geojsons: OutputHelper::Result.generate_geometry(job_object)
              }, with: VrpResult)
            end
            ...
            job_object = nil # rubocop:disable Lint/UselessAssignment
          end
          ...
          delete ':id' do # rubocop:disable Metrics/BlockLength
            ...
            if !job || job.killed? || Resque::Plugins::Status::Hash.should_kill?(id) || job['options']['api_key'] != params[:api_key]
              ...
              status 202
              if job_object && !job_object.empty?
                output_format = params[:format]&.to_sym || (job_object[:configuration] && job_object[:configuration][:csv] ? :csv : env['api.format'])
                if output_format == :csv
                  present(OutputHelper::Result.build_csv(job_object[:result]), type: CSV)
                else
                  present({
                    solutions: job_object[:result],
                    job: {
