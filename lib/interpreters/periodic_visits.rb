...
module Interpreters
  class PeriodicVisits
    ...
    def self.initialize
      @periods = []
      @equivalent_vehicles = {}
      @planning = {}     # pour chaque jour, liste des services à visiter, dans l'ordre dans lequel ils doivent être visités
      @indices = {}
      @order = []
    ...
    def self.adjust_candidate_routes(vehicle, day_finished, services, services_to_add, all_services)
      days_filled = []
        ...
        if peri && peri > 0
          ...
          (day_finished + peri..@schedule_end).step(peri).each{ |day|
            ...
            if @candidate_routes[vehicle].keys.include?(day) && !@vehicle_day_completed[vehicle][day] && nb_added < services[service[:id]][:nb_visits]
              ...
              if experiment[:current_route].last[:end] + matrix(@candidate_routes[vehicle][day], @candidate_routes[vehicle][day][:end_point_id], experiment[:current_route].last[:id] ) < @candidate_routes[vehicle][day][:tw_end]
    ...
    def self.compute_initial_solution(vrp)
      ...
      vrp.services.each{ |service|
        ...
        # Propagate the unavailable day indices following the heuristic logic
        if service[:unavailable_visit_day_indices] && !service[:unavailable_visit_day_indices].empty?
          Marshal::load(Marshal.dump(service[:unavailable_visit_day_indices])).each{ |day_index|
            step = services_data[service[:id]][:heuristic_period]
            if step
              (day_index..@schedule_end).each{ |equivalent_day_index|
                services_data[service[:id]][:unavailable_days] << equivalent_day_index
              }
            end
          }
          services_data[service[:id]][:unavailable_days].uniq!
        end
        if service[:timewindows]
          (service[:timewindows].collect{ |tw| tw.day_index }.uniq & (0..6).to_a).each{ |day_index|
            (day_index..@schedule_end).step(7) do |equivalent_day_index|
              services_data[service[:id]][:unavailable_days] << equivalent_day_index
            end
          }
        end
      ...
      generate_vehicles(vrp).each{ |vehicle|
        ...
        @candidate_routes[original_vehicle_id][vehicle[:global_day_index]] = {
          ...
          tw_start: vehicle[:timewindow][:start],
          tw_end: vehicle[:timewindow][:end],
      ...
      while !@candidate_vehicles.empty?
        ...
        while @candidate_service_ids.size > 0 && !current_day.nil?
          ...
          adjust_candidate_routes(current_vehicle, current_day, services_data, new_services, @planning[current_vehicle][current_day][:services])
