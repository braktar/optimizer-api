  ...
  if !ENV['SKIP_REAL_PERIODIC'] && !ENV['SKIP_PERIODIC']
    def check_quantities(vrp, solution)
      ...
      unit_ids.each{ |unit_id|
        ...
        solution.routes.each{ |route|
          ...
          route_quantities = route.activities.sum{ |act|
    ...
    def test_instance_baleares2
      ...
      assigned_service_ids = solutions[0][:routes].flat_map{ |route|
        route.activities.map(&:service_id)
      }.compact
    ...
    def test_vrp_allow_partial_assigment_false
      ...
      solutions[0].routes.each{ |route|
        route.activities.each_with_index{ |activity, index|
          ...
          next if route.activities[index + 1].timing.begin_time ==
                  route.activities[index + 1].detail.timewindows.first.start +
                  (route.activities[index + 1].timing.travel_time.positive? &&
                  route.activities[index + 1].detail.setup_duration || 0) # the same location

          assert_operator(
            route.activities[index + 1].timing.begin_time,
            :>=,
            activity.timing.departure_time +
              route.activities[index + 1].timing.travel_time +
              (route.activities[index + 1].timing.travel_time.positive? &&
                  route.activities[index + 1].detail.setup_duration || 0),
          )
        }
      }
    end
    ...
    def test_minimum_stop_in_route
      ...
      solutions = OptimizerWrapper.wrapper_vrp('ortools', { services: { vrp: [:ortools] }}, vrp, nil)
      assert solutions[0].routes.any?{ |r| r.activities.size - 2 < 5 },
             'We expect at least one route with less than 5 services, this test is useless otherwise'
      should_remain_assigned = solutions[0].routes.sum{ |r| r.activities.size - 2 >= 5 ? r.activities.size - 2 : 0 }

      # one vehicle should have at least 5 stops :
      vrp.vehicles.each{ |v| v.cost_fixed = 5 }
      vrp.services.each{ |s| s.exclusion_cost = 1 }
      solutions = OptimizerWrapper.wrapper_vrp('ortools', { services: { vrp: [:ortools] }}, vrp, nil)
      assert solutions[0].routes.all?{ |r| (r.activities.size - 2).zero? || r.activities.size - 2 >= 5 },
             'Expecting no route with less than 5 stops unless it is an empty route'
      assert_operator should_remain_assigned, :<=, (solutions[0].routes.sum{ |r| r.activities.size - 2 })
      assert_equal 19, solutions[0].unassigned.size

      all_ids = solutions[0].routes.flat_map{ |route| route.activities.map(&:service_id) }.compact +
    ...
    def test_performance_13vl
      ...
      vrps.each{ |vrp|
        ...
        seen += solutions[0].unassigned.size + solutions[0].routes.sum{ |r| r.activities.count(&:service_id) }
      }
    ...
    def test_fill_days_and_post_processing
      ...
      # voluntarily equal to watch evolution of periodic algorithm performance
      assert_equal 74, solutions[0].unassigned.size, 'Do not have the expected number of unassigned visits'
    end
    ...
    def test_treatment_site
      ...
      assert_equal(262, solutions[0].routes.count{ |r|
        r.activities.any?{ |a| a.service_id&.include? 'service_0_' } && r.vehicle.id
      }) # one treatment site per day
      assert(solutions[0][:routes].all?{ |r| r[:activities][-2].service_id.include? 'service_0_' })
      ...
      vrp[:services].each{ |s|
        ...
        days_used = solutions[0].routes.select{ |r|
          r.activities.any?{ |a| a.service_id&.include? "#{s.id}_" }
    ...
    def test_route_initialisation
      ...
      assert(solutions[0].routes.select{ |r|
        r.activities.any?{ |a| a.detail.point.id == '1000023' }
      }.all?{ |r| r.activities.any?{ |a| a.detail.point.id == '1000007' } })
      assert(solutions[0].routes.select{ |r|
        r.activities.any?{ |a| a.detail.point.id == '1000023' }
      }.all?{ |r| r.activities.any?{ |a| a.detail.point.id == '1000008' } })
    end
